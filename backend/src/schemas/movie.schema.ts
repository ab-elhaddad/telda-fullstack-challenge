...
export const createMovieSchema = Joi.object({
  ...
  poster: Joi.string().uri().max(500).allow('', null).messages({
    'string.uri': 'Poster must be a valid URL',
  }),

  rating: Joi.number().min(0).max(10).precision(1).allow(null).messages({
...
export const updateMovieSchema = createMovieSchema
  .fork(['title', 'director', 'release_year', 'genre', 'poster', 'rating'], schema =>
    schema.optional(),
  )
  .min(1)
...
export const movieQuerySchema = Joi.object({
  ...
  genre: Joi.string().trim().max(100).description('Filter by genre').messages({
  ...
  year_from: Joi.number()
    ...
    .messages({
      ...
      'number.max': `Starting year cannot be more than 5 years in the future (max: ${
        currentYear + 5
      })`,
    }),
  ...
  year_to: Joi.number()
    ...
    .messages({
      ...
      'number.max': `Ending year cannot be more than 5 years in the future (max: ${
        currentYear + 5
      })`,
    }),
  ...
  sort_by: Joi.string()
    .valid('title', 'release_year', 'rating', 'created_at')
    .default('created_at')
    .description('Field to sort results by')
    .messages({
      'any.only': 'Sort can only be by title, release_year, rating, or created_at',
    }),
